...
BreakoutGame.prototype.setNumMoves = function(value) 
{
	...
	try { window.parent.setStat('numMoves',value); } 
	catch(e) {}
...
BreakoutGame.prototype.setRunningTime = function(value) 
{
	...
	try { window.parent.setStat('runningTime',timeText); } 
	catch(e) {}
}
...
{	
	...
	if(x == undefined) x = $(this.content).width()/2;
	if(y == undefined) y = 0;
	
	if(vSpeed == undefined) vSpeed = 1;
	if(hSpeed == undefined) hSpeed = 0;
...
BreakoutGame.prototype.verifyGameComplete = function()
{
	...
	if(gameComplete)
	{
		...
		{
			$(barDisplay).remove();			
			try { window.parent.saveLogObject(logObject); } 
			catch(e) {}
		}
	}
